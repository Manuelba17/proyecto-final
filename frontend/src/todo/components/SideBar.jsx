...
export const SideBar = ({dispatch}) => {
  ...
  return (
    ...
    <div className='bg-slate-200 w-1/5 h-full flex flex-col gap-5 justify-between md:hidden xl:w-1/3'>
      ...
      <div className='w-full gap-5 p-2 cursor-pointer flex hover:bg-violet-200 place-items-center rounded-xl' onClick={() => startloadTasks(dispatch)}>
      <i className="fa-solid fa-house text-black fa-lg"></i>
      <span className='text-black text-lg'>Todas mis tareas</span>
      </div>

      <div className='w-full gap-5 p-2 cursor-pointer flex hover:bg-violet-200 place-items-center rounded-xl' onClick={() =>pendingTasks(dispatch)}>
      <i className="text-black fa-solid fa-triangle-exclamation fa-lg"></i>
      <span className='text-black text-lg'>Pendientes</span>
      </div>

      <div className='w-full gap-5 p-2 cursor-pointer flex hover:bg-violet-200 place-items-center rounded-xl' onClick={() => completedTasks(dispatch)}>
